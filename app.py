...
from text_preprocessing import preprocess_text_column, text_preprocessor
...
# Load the dataset
data_file = r"E:\Project\datasets\SQL script 3.csv"
...
# Load the saved XGBoost model and pipeline
model_file = 'E:\Project\models\xgb_pipeline.pkl'
...
menu_option = st.sidebar.radio('Select an option:', ['Dashboard', 'Preventive Measures', 'Chatbot', 'Risk Score'])
...
if menu_option == 'Dashboard':
...
elif menu_option == 'Risk Score':
    st.title('Predict Risk Score')
    patient_id_input = st.text_input('Enter PatientID:', '')
    ...
    if patient_id_input:
        ...
        if not patient_data.empty:
            ...
            required_columns = ['AdmissionID', 'PatientGender', 'PrimaryDiagnosisDescription', 'CBC_Lymphocytes', 
                                'CBC_Neutrophils', 'CBC_Basophils', 'CBC_Eosinophils', 'CBC_Hematocrit', 
                                'CBC_Hemoglobin', 'CBC_MCH', 'CBC_MCHC', 'CBC_Monocytes', 'CBC_PLATELET_COUNT', 
                                'CBC_RDW', 'CBC_RED_BLOOD_CELL_COUNT', 'CBC_WHITE_BLOOD_CELL_COUNT', 
                                'METABOLIC_ALBUMIN', 'METABOLIC_ALT_SGPT', 'METABOLIC_BILI_TOTAL', 'METABOLIC_BUN', 
                                'METABOLIC_CALCIUM', 'METABOLIC_CARBON_DIOXIDE', 'METABOLIC_CHLORIDE', 
                                'METABOLIC_CREATININE', 'METABOLIC_GLUCOSE', 'METABOLIC_POTASSIUM', 
                                'METABOLIC_SODIUM', 'METABOLIC_TOTAL_PROTEIN', 'URINALYSIS_PH', 
                                'URINALYSIS_RED_BLOOD_CELLS', 'URINALYSIS_WHITE_BLOOD_CELLS', 'Smoke', 'BMI', 
                                'Alcohol', 'Exercise', 'Age']
            ...
            if not selected_data.empty:
                st.subheader('Patient Data for Risk Score Prediction')
                st.write(selected_data[required_columns])

                # Prepare the data for prediction
                user_input_df = selected_data[required_columns]

                # Preprocess the user input using the loaded pipeline
                user_input_preprocessed = loaded_pipeline.named_steps['preprocessor'].transform(user_input_df)

                # Predict the risk score using the loaded model
                user_prediction = loaded_pipeline.named_steps['model'].predict(user_input_preprocessed)

                st.subheader(f'Predicted Risk Score: {int(user_prediction[0])}')
        else:
            st.error("PatientID not found. Please check the entered PatientID.")
